...
LINE_REPLACEMENTS: dict[int, str] = {
    ...
    54: r'''<p>Misalkan \(\bs{X} = (X_1, X_2, \ldots, X_n)\) merupakan <a href="Introduction.html">sampel acak</a> dari <a href="../special/Normal.html">distribusi normal</a> dengan <a href="../expect/Properties.html">rata-rata</a> \(\mu \in \R\) dan <a href="../expect/Variance.html">simpangan baku</a> \(\sigma \in (0, \infty)\). Ingat bahwa istilah <dfn>sampel acak</dfn> berarti bahwa \(\bs{X}\) adalah barisan variabel acak yang independen dan berdistribusi identik. Ingat pula bahwa distribusi normal memiliki fungsi densitas probabilitas''',
    ...
    93: r'''<p>Ingat bahwa jika \(\mu\) diketahui, penaksir yang wajar bagi varians \(\sigma^2\) adalah statistik''',
    ...
    109: r'''<p>Variabel \(U\) dalam <a href="#var1" class="ref"></a> berperan penting dalam menyusun <a href="../interval/Normal.html">estimasi interval</a> dan <a href="../hypothesis/Normal.html">uji hipotesis</a> untuk simpangan baku distribusi \(\sigma\) ketika rata-rata distribusi \(\mu\) diketahui (meskipun sekali lagi, asumsi ini biasanya tidak realistis).</p>''',
    ...
    158: r'''<p>Variabel \(V\) dalam <a href="#var4" class="ref"></a> berperan penting dalam menyusun <a href="../interval/Normal.html">estimasi interval</a> dan <a href="../hypothesis/Normal.html">uji hipotesis</a> untuk simpangan baku distribusi \(\sigma\) ketika rata-rata distribusi \(\mu\) tidak diketahui (seperti yang hampir selalu terjadi).</p>''',
    ...
    172: r'''<p>Seperti sebelumnya, hasil-hasil momen ini merupakan kasus khusus dari hasil umum yang diperoleh dalam bagian tentang <a href="Variance.html">Varians Sampel</a>. Sekali lagi, sebagai penaksir bagi \(\sigma^2\), bagian (a) berarti bahwa \(S^2\) takbias, sedangkan bagian (b) berarti bahwa \(S^2\) konsisten. Perhatikan pula bahwa \(\var(S^2)\) lebih besar daripada \(\var(W^2)\) (hal yang tidak mengejutkan), sebesar faktor \(\frac{n}{n - 1}\).</p>''',
    175: r'''\t<p class="app">Dalam <a href="JavaScript:openAncillary('../apps/SpecialSimulator.html')" class="ancillary">simulator distribusi khusus</a>, pilih distribusi khi-kuadrat. Ubah parameter derajat kebebasan dan perhatikan bentuk serta letak fungsi densitas probabilitas dan bilah rata-rata–simpangan baku. Untuk nilai parameter yang dipilih, jalankan eksperimen 1.000 kali dan bandingkan fungsi densitas empiris serta momen empiris dengan fungsi densitas probabilitas dan momen yang sebenarnya.</p>''',
    ...
    194: r'''<p>Ingat bahwa <a href="../special/Student.html">distribusi \(t\) Student</a> dengan \(k \in \N_+\) derajat kebebasan memiliki fungsi densitas probabilitas''',
    ...
    218: r'''\t<p class="app">Dalam <a href="JavaScript:openAncillary('../apps/SpecialSimulator.html')" class="ancillary">simulator distribusi khusus</a>, pilih distribusi \(t\). Ubah parameter derajat kebebasan dan perhatikan bentuk serta letak fungsi densitas probabilitas dan bilah rata-rata \( \pm \) simpangan baku. Untuk nilai parameter yang dipilih, jalankan eksperimen 1.000 kali dan bandingkan fungsi densitas empiris serta momen empiris dengan fungsi densitas dan momen distribusi.</p>''',
    ...
    241: r'''berdistribusi normal baku. Skor baku ini berperan mendasar dalam menyusun <a href="../interval/BivariateNormal.html">estimasi interval</a> dan <a href="../hypothesis/BivariateNormal.html">uji hipotesis</a> untuk selisih \(\mu - \nu\) ketika simpangan baku distribusi \(\sigma\) dan \(\tau\) diketahui.</p>''',
...
BACK_HALF_LINE_REPLACEMENTS: dict[int, str] = {
    ...
    381: r'''<p>Jika \(\mu\) dan \(\nu\) diketahui (sekali lagi, biasanya merupakan asumsi artifisial), penaksir yang wajar bagi kovarians distribusi \(\delta\) adalah bentuk khusus kovarians sampel''',
    ...
    396: r'''<p>Untuk ukuran sampel sekurang-kurangnya dua, jika \(\mu\) dan \(\nu\) tidak diketahui (sekali lagi, seperti yang biasanya terjadi), penaksir yang wajar bagi kovarians distribusi \(\delta\) adalah kovarians sampel baku''',
