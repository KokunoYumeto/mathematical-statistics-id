...
LINE_REPLACEMENTS = {
    ...
    242: r'''<p>Jika distribusi yang mendasarinya kontinu, kita dapat memberikan rumus sederhana bagi fungsi densitas probabilitas statistik terurut.</p>''',
    245: r'''\t<p class="math">Misalkan sekarang \(X\) memiliki <a href="../dist/Continuous.html">distribusi kontinu</a> dengan fungsi densitas probabilitas \(f\). Maka \(X_{(k)}\) memiliki distribusi kontinu dengan fungsi densitas probabilitas \(f_k\) yang diberikan oleh''',
    ...
    267: r'''\t<p class="math">Fungsi densitas probabilitas \(f_1\) dari \(X_{(1)}\) dan \(f_n\) dari \(X_{(n)}\) diberikan oleh</p>''',
    ...
    276: r'''<p>Kita kembali mengasumsikan bahwa \(X\) memiliki distribusi kontinu dengan fungsi distribusi \(F\) dan fungsi densitas probabilitas \(f\).</p>''',
    279: r'''\t<p class="math">Misalkan \(j,k \in \{1,2,\ldots,n\}\) dengan \(j \lt k\). Fungsi densitas probabilitas bersama \(f_{j,k}\) dari \(\left(X_{(j)},X_{(k)}\right)\) diberikan oleh</p>''',
    ...
    293: r'''\t<p class="math">Untuk ukuran sampel sedikitnya dua, fungsi densitas probabilitas bersama \(f_{1,n}\) dari \(\left(X_{(1)},X_{(n)}\right)\) diberikan oleh''',
    296: r'''\t\t<summary>Rincian:</summary>''',
    297: r'''\t\t<p>Ini merupakan akibat <a href="#jnt1" class="ref"></a> dengan \(j=1\) dan \(k=n\).</p>''',
    301: r'''<p>Argumen serupa dapat digunakan untuk memperoleh fungsi densitas probabilitas bersama untuk sembarang banyak statistik terurut. Tentu saja, kita terutama tertarik pada fungsi densitas probabilitas bersama dari <em>semua</em> statistik terurut. Ternyata fungsi densitas tersebut memiliki bentuk yang sangat sederhana.</p>''',
    304: r'''\t<p class="math">\(\left(X_{(1)}, X_{(2)}, \ldots, X_{(n)}\right)\) memiliki fungsi densitas probabilitas bersama \(g\) yang diberikan oleh''',
    ...
    309: r'''\t\t<p>Sekali lagi, terdapat argumen heuristik sederhana. Untuk setiap \(\bs{x} \in \R^n\) dengan \(x_1 \lt x_2 \lt \cdots \lt x_n\), ada \(n!\) permutasi koordinat \(\bs{x}\). Densitas probabilitas \((X_1,X_2,\ldots,X_n)\) pada setiap titik itu ialah \(f(x_1)f(x_2)\cdots f(x_n)\). Jadi, densitas probabilitas \((X_{(1)},X_{(2)},\ldots,X_{(n)})\) pada \(\bs{x}\) adalah \(n!\) kali hasil kali tersebut.</p>''',
    ...
    524: r'''\t<p class="math">Misalkan \(\bs{X}\) adalah sampel acak berukuran \(n\) dari distribusi seragam baku. Untuk \(k \in \{1,2,\ldots,n\}\), \(X_{(k)}\) memiliki <a href="../special/Beta.html">distribusi beta</a> dengan parameter kiri \(k\) dan parameter kanan \(n-k+1\). Fungsi densitas probabilitas \(f_k\) diberikan oleh''',
    ...
    553: r'''\t<p class="math">Misalkan \(\bs{X}\) adalah sampel acak berukuran \(n \ge 2\) dari distribusi seragam baku. Rentang sampel \(R\) memiliki distribusi beta dengan parameter kiri \(n-1\) dan parameter kanan 2. Fungsi densitas probabilitas \(g\) diberikan oleh''',
    556: r'''\t\t<summary>Rincian:</summary>''',
    557: r'''\t\t<p>Dari <a href="#jnt2" class="ref"></a>, fungsi densitas probabilitas bersama \((X_{(1)},X_{(n)})\) adalah \(f_{1,n}(x,y)=n(n-1)(y-x)^{n-2}\) untuk \(0 \le x \le y \le 1\). Karena itu, untuk \(r \in [0,1]\),''',
    559: r'''\t\tDengan demikian, fungsi distribusi \(R\) adalah \(G(r)=nr^{n-1}-(n-1)r^n\) untuk \(0 \le r \le 1\). Mengambil turunan terhadap \(r\) dan menyederhanakannya memberikan fungsi densitas \(g(r)=n(n-1)r^{n-2}(1-r)\) untuk \(0 \le r \le 1\). Dari bentuk \(g\), distribusi ini adalah distribusi beta dengan parameter kiri \(n-1\) dan parameter kanan 2.</p>''',
    ...
    583: r'''\t\t<p>Hasil ini langsung mengikuti fakta bahwa \((X_1,X_2,\ldots,X_n)\) berdistribusi seragam pada \([a,a+h]^n\). Dari <a href="#jnt3" class="ref"></a>, fungsi densitas probabilitas bersama statistik terurut adalah \(g(x_1,x_2,\ldots,x_n)=n!/h^n\) untuk \((x_1,x_2,\ldots,x_n) \in [a,a+h]^n\) dengan \(a \le x_1 \le x_2 \le \cdots \le x_n \le a+h\).</p>''',
    587: r'''<h4 id="exp">Distribusi Eksponensial</h4>''',
    589: r'''<p>Ingat bahwa <a href="../poisson/Exponential.html">distribusi eksponensial</a> dengan parameter laju \(\lambda \gt 0\) memiliki fungsi densitas probabilitas''',
    ...
    594: r'''\t<p class="math">Misalkan \(\bs{X}\) adalah sampel acak berukuran \(n\) dari distribusi eksponensial dengan parameter laju \(\lambda\). Fungsi densitas probabilitas dari statistik terurut ke-\(k\), \(X_{(k)}\), adalah''',
    ...
    599: r'''\t\t<p>Fungsi densitas probabilitas dari \(X_{(k)}\) mengikuti <a href="#dst4" class="ref"></a> karena \(F(x)=1-e^{-\lambda x}\) untuk \(0 \le x \lt \infty\). Dengan menetapkan \(k=1\), diperoleh \(f_1(x)=n\lambda e^{-n\lambda x}\) untuk \(0 \le x \lt \infty\).</p>''',
    ...
    608: r'''\t<p class="math">Misalkan kembali \(\bs{X}\) adalah sampel acak berukuran \(n \ge 2\) dari distribusi eksponensial dengan parameter laju \(\lambda\). Rentang sampel \(R\) memiliki distribusi yang sama dengan maksimum sampel acak berukuran \(n-1\) dari distribusi eksponensial. Fungsi densitas probabilitasnya adalah''',
    609: r'''\t\[ h(t) = (n - 1) \lambda (1 - e^{-\lambda t})^{n - 2} e^{-\lambda t}, \quad 0 \le t \lt \infty \]''',
    611: r'''\t\t<summary>Rincian:</summary>''',
    612: r'''\t\t<p>Menurut <a href="#jnt2" class="ref"></a>, \((X_{(1)},X_{(n)})\) memiliki fungsi densitas probabilitas bersama \(f_{1,n}(x,y)=n(n-1)\lambda^2(e^{-\lambda x}-e^{-\lambda y})^{n-2}e^{-\lambda x}e^{-\lambda y}\) untuk \(0 \le x \le y \lt \infty\). Karena itu, untuk \(0 \le t \lt \infty\),''',
    ...
    616: r'''\t\tMenurunkannya terhadap \(t\) memberikan fungsi densitas probabilitas. Dengan membandingkannya dengan <a href="#exp1" class="ref"></a>, terlihat bahwa ini adalah fungsi densitas probabilitas dari maksimum suatu sampel berukuran \(n-1\) dari distribusi eksponensial.</p>''',
    621: r'''\t<p class="math">Misalkan kembali \(\bs{X}\) adalah sampel acak berukuran \(n\) dari distribusi eksponensial dengan parameter laju \(\lambda\). Fungsi densitas probabilitas bersama statistik terurut \((X_{(1)},X_{(2)},\ldots,X_{(n)})\) adalah''',
    ...
    721: r'''\t<p class="app">Dalam <a href="https://www.randomservices.org/random/apps/ProbabilityPlot.html" class="ancillary">eksperimen plot probabilitas</a>, atur distribusi pengambilan sampel menjadi distribusi normal dengan rata-rata 5 dan simpangan baku 2. Atur ukuran sampel menjadi \(n = 20\). Untuk setiap distribusi uji berikut, jalankan eksperimen 50 kali dan perhatikan bentuk geometris plot probabilitas:</p>''',
    723: r'''\t\t<li>Normal baku</li>''',
    724: r'''\t\t<li>Seragam pada interval \([0, 1]\)</li>''',
    725: r'''\t\t<li>Eksponensial dengan parameter 1</li>''',
    730: r'''\t<p class="app">Dalam <a href="https://www.randomservices.org/random/apps/ProbabilityPlot.html" class="ancillary">eksperimen plot probabilitas</a>, atur distribusi pengambilan sampel menjadi distribusi seragam pada \([4, 10]\). Atur ukuran sampel menjadi \(n = 20\). Untuk setiap distribusi uji berikut, jalankan eksperimen 50 kali dan perhatikan bentuk geometris plot probabilitas:</p>''',
    732: r'''\t\t<li>Normal baku</li>''',
    733: r'''\t\t<li>Seragam pada interval \([0, 1]\)</li>''',
    734: r'''\t\t<li>Eksponensial dengan parameter 1</li>''',
    739: r'''\t<p class="app">Dalam <a href="https://www.randomservices.org/random/apps/ProbabilityPlot.html" class="ancillary">eksperimen plot probabilitas</a>, atur distribusi pengambilan sampel menjadi distribusi eksponensial dengan parameter 3. Atur ukuran sampel menjadi \(n = 20\). Untuk setiap distribusi uji berikut, jalankan eksperimen 50 kali dan perhatikan bentuk geometris plot probabilitas:</p>''',
