#!/usr/bin/env python3
"""Advance the existing O006 Zenodo concept to the verified 14/29 checkpoint."""
...
CURRENT_RECORD_ID = "22059764"
CONCEPT_RECORD_ID = "22059763"
PUBLISHED_RECORD_ID = "22061677"
VERSION = "2026.08.22.14"
RELEASE_DIR = ROOT / "release" / "zenodo" / VERSION
FILES = (
    "00_statistika-matematis-id-reader-2026.08.22.14.pdf",
    "10_mathematical-statistics-id-2026.08.22.14-reader-html.zip",
    "20_mathematical-statistics-id-2026.08.22.14-source-provenance.zip",
    "30_mathematical-statistics-id-2026.08.22.14-modular-backend.zip",
    "40_LICENSE.md",
    "50_mathematical-statistics-id-2026.08.22.14-release-manifest.json",
    "SHA256SUMS.txt",
)
...
def metadata() -> dict[str, object]:
    return {
        "title": "Statistika Matematis — Edisi Bahasa Indonesia (id-ID): Checkpoint 14 dari 29 (Belum Lengkap)",
        ...
        "description": (
            ...
            "Processes. Versi ini memuat 14 dari 29 halaman sumber inti secara berurutan, "
            "sampai random/point/Bayes.html; korpus belum lengkap dan tidak diklaim sebagai "
            "satu-satunya tulang punggung naratif C140. Berkas pertama adalah pembaca PDF "
            "langsung 173 halaman A4. Pembaca HTML luring, sumber terjemahan yang dapat "
            "disunting, backend modular netral-lokal, manifes, provenance ringkas, lisensi, "
            "checksum, dan bukti QA turut disertakan. Situs asal menyatakan CC BY 2.0, "
...
def ensure_draft(session: requests.Session) -> tuple[dict[str, object], bool]:
    ...
    if not current.get("submitted") or str(current.get("record_id")) != CURRENT_RECORD_ID:
        raise RuntimeError("current Zenodo record is not the expected submitted v13")
    response = check(
...
def anonymous_readback(record_id: str, inventory: list[dict[str, object]]) -> dict[str, object]:
    ...
    if len(hits) != 2:
        raise RuntimeError(f"expected two public versions, found {len(hits)}")
    return {
...
def main() -> None:
    ...
    if args.preflight:
        ...
        return
    if args.verify_published:
        public = anonymous_readback(PUBLISHED_RECORD_ID, inventory)
        final_lineage = search_lineage(session)
        if sum(bool(row.get("submitted")) for row in final_lineage) != 2 or any(
            not bool(row.get("submitted")) for row in final_lineage
        ):
            raise RuntimeError(
                "authenticated Zenodo lineage is not two submitted versions and zero drafts"
            )
        summary.update(
            {
                "final_depositions": len(final_lineage),
                "public": public,
                "submitted_versions": 2,
    ...
    if sum(bool(row.get("submitted")) for row in final_lineage) != 2 or any(
        ...
        raise RuntimeError("authenticated Zenodo lineage is not two submitted versions and zero drafts")
    summary.update(
        {
            "draft_id": draft_id,
            "draft_reused": reused,
